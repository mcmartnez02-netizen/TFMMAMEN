...
class ObjectTracking:
    ...
    def __init__(self, model="yolo26n-pose.pt", source="path/to/video.mp4"):
        ...
        with open(CONFIG_FILE, mode="rb") as fp:
            ...
            self.near_threshold = float(config['detection']['near_height'])            
            self.far_threshold = float(config['detection']['far_height'])
            self.conf_detection_threshold = float(config['detection']['confidence_threshold'])

    def draw_bbox(self, im0, box, track_id, height: float, near:bool):
        ...
        # Prepare label
        label = f"{int(track_id):}:{float(height):.2f}:{'N' if near else 'F'}"
