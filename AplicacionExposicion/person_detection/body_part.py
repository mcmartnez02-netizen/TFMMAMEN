...
def calculateHeight(keypoints: list[list[float]], conf:list[float]) -> float | None:
    ...
    for level, value in enumerate(DETECTION_LEVELS):
        name, p1_indexes, p2_indexes = value
        ...
        if p1 != None and p2 != None:            
            print(level, p1, p2, "|", math.dist(p1, p2),"*", factor, "=",math.dist(p1, p2) * factor)
...
if __name__ == "__main__":
    ...
    test_keypoints =[[184.0959,  56.0840],
         ...
         [243.3983, 500.7520],
         [175.7904, 501.1375]]
    conf = [0.9978, 0.9991, 0.9819, 0.9882, 0.1985, 0.9977, 0.9991, 0.9848, 0.9930, 0.9958, 0.9920, 0.9988, 0.9990, 0.9983, 0.9986, 0.8379, 0.8712]
    calculateHeight(test_keypoints, [0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5])
