...
class Heartbeat(BaseModel):
    ...
    def __iter__(self):
        yield from [
            (k, v) if not type(v) is HB_State else (k, v.name)
...
class HeartbeatBatch(BaseModel):
    run_id: int
    items: list[Heartbeat] = Field(min_length=0, max_length=500)
...
class User(BaseModel):
    ...
    def convert_scopes(cls, v):
        if isinstance(v, list):
            return [
                AvailableScopes[item] if isinstance(item, str) else item for item in v
            ]
        return v
